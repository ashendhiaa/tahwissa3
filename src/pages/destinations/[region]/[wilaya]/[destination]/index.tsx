          ...
          if (index === 0) {
            return (
              <img
                ...
                src={picture.url}
                alt={picture.name}
                className="image col-span-2 row-span-2"
              />
            );
          } else {
            return (
        ...
        <div id="about" className="mt-[3.385vw] flex">
          ...
          >
            ...
            <div id="hours" className="mt-[2.083vw] flex items-center">
              ...
              <p className="detail ml-[1.042vw]">Visit hours: 8AM - 7PM</p>
            </div>
            <div id="price" className="mt-[2.083vw] flex items-center">
              ...
              <p className="detail ml-[1.395vw]">Price: DZD {site.price}</p>
            </div>
            <div className="detail col mt-[2.34375vw] aspect-[5.316] w-[20.9vw] justify-center rounded-[1.953vw] bg-newBlack text-center font-bold text-white ">
...
const Site = ({
  ...
  const site = top?.find((site) => site.name === destination);
  const [siteImages, setSiteImages] = useState<any[]>([]);

  const [hotels, setHotels] = useState([]);
  const [restaurants, setRestaurants] = useState([]);

  const listRecommend = top?.filter((site) => site.name !== destination);
  ...
  return (
    <main className="col mx-auto w-[87.1vw] gap-[8.854166666666668vw]">
      <SiteHero site={site} images={siteImages} />
      <section
        id="Map"
        className="col mt-[0.6510416666666667vw] gap-[3.3854166666666665vw] "
      >
        <h2 className="semi-title">Near {site.name}</h2>
        <Map center={site.position as [number, number]} zoom={15}></Map>
      </section>
      <SectionPlaces
        data={listRecommend}
...
const FetcherComponent = () => {
  ...
  let { region, wilaya, destination } = query;
  ...
  const destinationPhrase = destination.split(" ");
  ...
  const destinationCapital = destinationPhrase.map((word) => {
    return word.charAt(0).toUpperCase() + word.slice(1);
  });
  ...
  const destinationCapitalString = destinationCapital.join(" ");
  ...
  return (
    <SiteFetcher
      ...
      wilayaName={wilayaCapitalString}
      destination={destinationCapitalString}
    />
  );
};
